      ...
      if get_pwd != get_repwd:
        ...
        if not re.match("(?=.*\d).{10}", get_phone):
          ...
          try:
            ...
            cursor = con.cursor()
            cursor.execute("Insert into admin_login values('" + get_username + "','" + get_name + "','" + get_branch + "','" + get_phone + "','" + get_pwd + "')")
...
frame1 = Frame(pw, relief=FLAT, bg="#4169E1")
frame2 = Frame(pw, relief=FLAT, bg="#fff")
